# The right claims find me

At the moment of acting — mine or my agent's — the claims that bear on
the act surface themselves, unasked. My live practice runs on this: a
path like `must-read.kb/before/running-ANY-Bash-commands.md` names its
own trigger, and `requires:` frontmatter makes the read an obligation,
not a suggestion. Push is how my corpora actually get read; a corpus
that must be queried to be heard goes unread like any reference doc.
