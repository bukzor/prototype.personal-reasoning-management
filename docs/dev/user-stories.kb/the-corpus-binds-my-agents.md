# The corpus binds my agents

Most reads and writes against my corpus are by agents working on my
behalf, mid-task. A claim is not only a record for me — it is an
obligation on the next actor: my `design-rules.kb` and `must-read.kb`
already function this way, and this repo's own `conduct` theory is
