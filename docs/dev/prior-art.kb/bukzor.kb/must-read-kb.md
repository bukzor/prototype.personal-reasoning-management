# must-read.kb — trigger banks, mandates in the wild

`Skill(llm-must-read-kb)` and the live `~/.claude/must-read.kb/`: a
`.kb/` with a prescribed access pattern — the filename names its own
trigger condition (`before/git/commit.md`), agents scan the listing
while planning and read a body only when its trigger fires; `requires:`
frontmatter makes reads hard dependencies of acts. This is
mandates-with-authority running today, on convention alone.
`Skill(llm-triggers)` is its designed successor.

**Gems:**

- Prescribed access: `ls -RF` once buys a passive index of every
  mandate; filenames are cheap to scan and expensive to ignore. The
  right claims finding the agent, at zero mechanism —
