...
- IMAGE!: The ledger is apparently its own first instance — the artifact recording the design is built from the design, at lower fidelity. An observation, not a commitment: it may lawfully become false for good reason. Until then, a term appearing at two fidelities is aligned, not colliding, and a divergence is suspicious — investigated and explained, never shrugged off -- authority: user
...
- SIGIL_RENDER <- IMAGE!, PROJECTION, ASSERT!: A sigil marks a stipulation-shaped claim — one with residual choice its premises don't settle: `!` a ruled or certified choice, `+` a noticer's stipulation awaiting veto, `?` an open question. An asserted claim stays bare no matter who has agreed with it — its standing is computed from its premises, and painting that onto the line would store a projection. At this fidelity sigils are hand-authored; in the design they are derived, so per IMAGE! a sigil on a claim that rests entirely on its premises is the smell, and the veto scan stays a grep because the mark rides the label's own line
- HASH_RENDER <- ARROWS!, ID_BOUNDARY!: On a ledger line the hashed statement is the text after the colon; label, premise arrows, sigil, and authority trailer stand outside identity — renaming, re-motivating, or re-attributing a claim strands no verdict
