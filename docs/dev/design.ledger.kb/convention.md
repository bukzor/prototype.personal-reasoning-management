...
- SIGIL_RENDER <- IMAGE!, PROJECTION: A sigil is the image's rendering of its claim's evidence set — who has weighed in, projected to one trailing character. At this fidelity it is hand-authored; in the design it is derived, so per IMAGE! a sigil diverging from the evidence it summarizes is the smell, and the veto scan stays a grep because the projection rides the label's own line
