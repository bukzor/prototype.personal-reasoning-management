...
- LEAN_REALIZE+ <- DECISION!, HASH_FORMAL, FRAME_BUNDLE+, CMD_SURFACE, NO_SIDECAR: Every `host` commitment's Lean4 realization, collected so `host` stays substrate-free — hashing: pre-elaboration `Syntax` tree; frames: explicit `structure`s; verbs: `elab` commands; persistence: `PersistentEnvExtension` in the `.olean`s; audit: lean4export + lean4checker. One facet, one file: `world.kb/lean-realize.kb/`
