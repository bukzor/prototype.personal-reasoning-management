...
/-- Emit the formal artifacts a claim's `stmt:`/`proof:` lines author
(`DEPTH_ATTEST`). Adjacency to the row is the pin -- the elaborator makes a
missing or ill-typed artifact a build failure. The statement is `abbrev` so
proofs and `decide` can see through the name. -/
def emitArtifacts (e : Entry) : List String :=
  let stmtDecl := e.stmt.map fun s =>
...
abbrev C.{e.label}.stmt : Prop := {s}"
  let pfDecl := e.proofTactic.map fun p =>
    s!"/-- Proof authored for `{e.label}`: `stmt` discharged. -/
theorem C.{e.label}.pf : C.{e.label}.stmt := by {p}"
  stmtDecl.toList ++ pfDecl.toList

/-- The `Depth` cell an entry's authored artifacts pay for. -/
def depthCell (e : Entry) : String :=
  match e.stmt, e.proofTactic with
  | none, none => ".stated"
  | some _, none => s!".described C.{e.label}.stmt"
  | some _, some _ => s!".proven C.{e.label}.stmt C.{e.label}.pf"
...
def emit (entries : Array Entry) : String :=
  let decls := entries.toList.flatMap fun e => emitArtifacts e ++ [emitRow e]
  let cells := entries.toList.map fun e =>
    s!"  \{ row := C.{e.label}, depth := {depthCell e} }"
  String.intercalate "\n\n" <| (
